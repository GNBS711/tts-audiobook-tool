# tts_audiobook_tool/voice_menu/voice_omnivoice_menu.py
...
def ask_num_step(project: Project) -> None:
    prompt = f"Enter number of diffusion steps {COL_DIM}(recommended: 4-64; default 32; use 16 for 2x faster){COL_DEFAULT}: "
    inp = AskUtil.ask(prompt, lower=False)
    if not inp:
        return
    try:
